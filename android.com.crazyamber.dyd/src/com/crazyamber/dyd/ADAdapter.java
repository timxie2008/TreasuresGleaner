...
public class ADAdapter implements AdsMogoListener
{
	...
	private void _rebuildAD()
	{
		try
		{
			...
			if (!_config.bEnableADMenu && !_config.bEnableADPlay)
			{
				//no ad 
				return;
			}
	...
	private void _updateADConfig()
	{
		...
		{
			//fill default params
			_config.bEnableADMenu = false;
			_config.bEnableADPlay = false;
	...
	private void _showKuguoSprite()
	{
		String kugo = Settings.get(_context, "kugo");
		String id = _c.decryptString(kugo);
		
		Logger.d(TAG, "=== kid=" + id);
		
		KuguoAdsManager.getInstance().setCooId(_context, id);
		
		KuguoAdsManager km = KuguoAdsManager.getInstance();
		km.showKuguoSprite(_context, 0);
		km.setKuzaiPosition(true, 0);
	}
