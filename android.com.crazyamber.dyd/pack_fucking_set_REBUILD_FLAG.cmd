...
for /D %%i in (../publish/pushversion/*) do (
	...
	if exist ..\publish\pushversion\%%i\dyd_%%i_%1.apk (
		echo checking
		if %REMOVEALL% EQU 1 (
			set REBUILD=1
		) else (
			...
			echo skip ..\publish\pushversion\%%i\dyd_%%i_%1.apk
		)
	) else (
		set REBUILD=1
	)
	...
	if %REBUILD% EQU 1 (
		echo rebuilding ..\publish\pushversion\%%i\dyd_%%i_%1.apk
		...
		echo export apk file:..\publish\pushversion\%%i\dyd_%%i_%1.apk
		rem dir .\bin\com.crazyamber.dyd-release.apk
		rem for %%f in (.\bin\com.crazyamber.*.apk) do (
		copy .\bin\com.crazyamber.dyd-release.apk ..\publish\pushversion\%%i\dyd_%%i_%1.apk /Y
		rem )
		rem pause
	)
)
